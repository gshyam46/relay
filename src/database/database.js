...
export function createDatabase(databaseFile = ":memory:") {
  const db = new SqliteDatabaseClient(databaseFile);
  migrateDatabase(db);
  return db;
}
...
function migrateDatabase(db) {
  ...
  ensureColumn(db, "leads", "normalized_email", "TEXT");
  ensureColumn(db, "leads", "normalized_phone", "TEXT");
  ensureColumn(db, "leads", "import_batch_id", "TEXT");
  ensureColumn(db, "leads", "import_row_id", "TEXT");
  ensureColumn(db, "leads", "source_metadata_json", "TEXT NOT NULL DEFAULT '{}'");
  ensureColumn(db, "actions", "next_best_action_plan_id", "TEXT");
  ensureColumn(db, "actions", "approval_requirement", "TEXT NOT NULL DEFAULT 'NOT_REQUIRED'");
  ensureColumn(db, "actions", "execution_mode", "TEXT NOT NULL DEFAULT 'SANDBOX'");
  ensureColumn(db, "actions", "provider", "TEXT NOT NULL DEFAULT 'mock-n8n'");
  ensureColumn(db, "actions", "last_error", "TEXT");
  ensureColumn(db, "callbacks", "organization_id", "TEXT");
  ensureColumn(db, "callbacks", "lead_id", "TEXT");
  ensureColumn(db, "callbacks", "action_execution_id", "TEXT");
  ensureColumn(db, "callbacks", "status", "TEXT NOT NULL DEFAULT 'RECEIVED'");
  ensureColumn(db, "callbacks", "provider_reference", "TEXT");
  ensureColumn(db, "intelligence_snapshots", "version", "INTEGER NOT NULL DEFAULT 1");
  ensureColumn(db, "intelligence_snapshots", "status", "TEXT NOT NULL DEFAULT 'READY'");
  ensureColumn(db, "intelligence_snapshots", "pipeline_version", "TEXT NOT NULL DEFAULT 'm2.0-deterministic-v1'");
  ensureColumn(db, "intelligence_snapshots", "input_fingerprint", "TEXT");
  ensureColumn(db, "intelligence_snapshots", "readiness_status", "TEXT NOT NULL DEFAULT 'NEEDS_MORE_DATA'");
  ensureColumn(db, "intelligence_snapshots", "readiness_score", "INTEGER NOT NULL DEFAULT 0");
  db.run("UPDATE leads SET normalized_email = lower(trim(email)) WHERE normalized_email IS NULL AND email IS NOT NULL");
  db.run("UPDATE leads SET normalized_phone = phone WHERE normalized_phone IS NULL AND phone IS NOT NULL");
  db.run("UPDATE leads SET source_metadata_json = '{}' WHERE source_metadata_json IS NULL");
  db.exec(`
