...
function AuthGate(){const query=useSessionBoundary(),{data:me,isLoading}=query,location=useLocation();if(isLoading)return loading;if(query.isError&&isServiceUnavailable(query.error))return <AvailabilityNotice source="ONBOARDING" comingSoon={isComingSoon(query.error)} onRetry={()=>query.refetch()}/>;if(!me)return <Navigate replace to={"/login?returnTo="+encodeURIComponent(location.pathname+location.search+location.hash)}/>;return <Outlet/>;}
...
function AuthRoute({mode}:{mode:"login"|"register"}){const query=useSessionBoundary(),{data:me,isLoading}=query,location=useLocation();if(isLoading)return loading;if(query.isError&&isServiceUnavailable(query.error))return <AvailabilityNotice source={mode==="register"?"SIGNUP":"SIGNIN"} comingSoon={isComingSoon(query.error)} onRetry={()=>query.refetch()}/>;if(me)return <Navigate replace to={safeReturn(new URLSearchParams(location.search).get("returnTo"))}/>;return <AuthPage key={mode} initialMode={mode}/>;}

function PublicFallback(){return <main className="mx-auto w-full max-w-5xl px-6 py-16"><a href="/" aria-label="Relay home" className="mb-10 inline-flex items-center gap-3"><BrandMark className="h-9 w-9"/><span className="brand-wordmark text-2xl">Relay.</span></a><p className="text-sm text-muted">AI Lead Intelligence &amp; Outbound Automation</p><h1 className="my-6 font-display text-5xl font-normal leading-tight">Your next move, backed by what you know.</h1><p>Turn existing enquiries into explained priorities and reviewed actions. Product preview; examples use synthetic data.</p><nav className="mt-8 flex gap-5"><a href="/" className="underline">Reload page</a><a href="/login" className="underline">Sign in</a><a href="/register" className="underline">Get started</a></nav></main>;}
