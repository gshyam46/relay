...
export function AppShell() {
  ...
  return <div className="flex h-dvh w-full flex-col overflow-hidden md:flex-row">
    <Sidebar />
    <div className="flex min-h-16 shrink-0 items-center justify-between gap-3 border-b border-line bg-page px-4 md:hidden"><a href="/app" className="inline-flex min-h-11 items-center gap-2 font-semibold text-ink"><BrandMark className="h-6 w-6 text-brand" /><span className="text-xl tracking-[-0.06em]">Relay<span className="text-brand">.</span></span></a><button ref={trigger} type="button" aria-label="Open workspace navigation" aria-haspopup="dialog" aria-controls="mobile-workspace-navigation" aria-expanded={open} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-line px-3 text-sm focus-visible:outline-2 focus-visible:outline-brand" onClick={() => setOpen(true)}><Menu size={18} aria-hidden="true" />Menu</button></div>
    ...
    <dialog ref={dialog} id="mobile-workspace-navigation" aria-label="Workspace navigation" className="m-0 h-dvh max-h-none w-[min(20rem,90vw)] max-w-none border-0 bg-sidebar p-0 text-sidebar-text backdrop:bg-ink/30" onKeyDown={keepFocus} onCancel={event => { event.preventDefault(); setOpen(false); }} onClose={closed} onClick={event => { if (event.target === event.currentTarget) setOpen(false); }}><Sidebar mobile onNavigate={() => setOpen(false)} /></dialog>
  </div>;
}
