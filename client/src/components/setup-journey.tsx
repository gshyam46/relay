...
export function SetupJourney() {
  ...
  return <section aria-label="Your first enquiry journey" className="rounded-xl border border-line bg-surface p-5 space-y-4">
    ...
    {query.data && <><div className="rounded-lg bg-soft p-3 text-sm"><p>{query.data.lead ? <>Enquiry in this guide: <Link className="font-medium underline" to={"/leads/" + encodeURIComponent(query.data.lead.id)}>{query.data.lead.name || "Unnamed enquiry"}</Link>. This is the latest active enquiry, excluding channel-verification probes.</> : "No active enquiry is selected. Add one or review an import to start."}</p><p className="mt-1 text-xs text-muted">Read at {new Date(query.data.assessed_at).toLocaleString()}. Other enquiries may be at different stages.</p></div>
      <ol className="grid gap-x-8 lg:grid-cols-2">{query.data.steps.map((row, index) => { const link = row.id === "BUSINESS" && row.details.configured && !row.details.criteria_configured ? { href: "/settings?tab=fit", label: "Choose fit criteria" } : destination(row.id, query.data?.lead?.id), Icon = row.state === "RECORDED" ? Check : row.state === "NOT_STARTED" ? Circle : TriangleAlert; return <li key={row.id} aria-label={titles[row.id]} className="flex min-w-0 gap-4 border-b border-line py-5 last:border-b-0"><span className="flex h-6 w-5 shrink-0 items-center justify-center text-[11px] font-medium tabular-nums text-brand" aria-hidden="true">{index + 1}</span><div className="min-w-0 space-y-2"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-semibold">{titles[row.id]}</h3><span className="inline-flex items-center gap-1 rounded-full bg-soft px-2 py-1 text-[11px]"><Icon size={12} aria-hidden="true" />{labels[row.state]}</span></div><p className="text-xs leading-6 text-muted">{explanation(row)}</p><Link className="inline-flex min-h-10 items-center gap-1 text-xs font-semibold text-brand underline-offset-4 hover:underline" to={link.href}>{link.label}<ArrowUpRight size={14} aria-hidden="true" /></Link>{row.id === "ENQUIRY" && <Link to="/leads" className="ml-3 inline-flex min-h-10 items-center text-xs text-brand underline-offset-4 hover:underline">Add or correct an enquiry</Link>}</div></li>; })}</ol>
    </>}
  </section>;
}
