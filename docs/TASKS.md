...
## Restore the lively landing composition - 2026-09-15

Locally implemented and verified under the latest user correction, within L4-07. Restore the pre-theme hero, layered cards, section contrast and interactions from commit 86331d6, retaining the ivory/unicorn-blue palette, current branding, larger supporting text and visual control cues. Dependencies: existing landing component/styles, public CTA and loading contracts. Acceptance: the earlier composition is recognizable at desktop and phone sizes; the synthetic walkthrough, keyboard/reduced-motion behavior, contrast, no-overflow, registration and saved request flow continue to pass. Automated verification: TypeScript, production build and existing landing browser suite. Human QA: user visual acceptance plus actual-device/screen-reader checks remain separate. No API/schema changes. The final build passed all 27 existing landing browser scenarios; desktop, tablet and phone diagnostics passed after correcting hero-caption spacing. See [restoration evidence](verification/PREMIUM_THEME.md#restore-the-earlier-landing-experience---2026-09-15). User visual acceptance remains open.

## Landing motion enhancement - 2026-09-15

Locally implemented and verified within L4-07 under the latest user request. Add native motion to the restored landing and existing hero, sections, controls and walkthrough while preserving the ivory/blue composition. Acceptance: visible meaningful transitions, bounded decorative motion, responsive/touch layouts, live reduced-motion support, readable fallback without animation APIs, stable keyboard focus and unchanged demo/CTA/provider contracts. Verification: TypeScript/build, existing landing regression suite plus focused browser coverage for the new motion lifecycle and fallback; inspect desktop/phone motion and update theme evidence. User visual acceptance and actual-device/screen-reader QA remain separate.

Motion completion: TypeScript/build, formatting and all 31 landing browser scenarios passed. Four added checks cover native motion, live/initial reduced motion and progressive API fallbacks. The initial step-number crossfade contrast failure was corrected before the final pass. See [motion evidence](verification/PREMIUM_THEME.md#landing-motion-enhancement---2026-09-15).
