...
## Motion enhancement - 15 September 2026

The user requests more animation on the restored landing and its existing elements. Use native CSS/SVG and the Web Animations API: staggered section entrances, a finite hero-card settle and orbit trace, small fine-pointer depth offsets, button/link/card feedback, FAQ/menu entrances and walkthrough state transitions. Preserve layout, colours, copy and data contracts. Decorative entrances finish within five seconds; no automatic demo advancement or endless attention animation. Content is visible by default, including when observation or animation APIs fail. Motion cancels when reduced-motion preference changes; decorative hero work stops offscreen or in a hidden tab. Keyboard focus must immediately reveal stable content; touch gets usable controls without pointer dependence. No new runtime dependency. Root owns the lifecycle hook, TSX integration, docs and verification; separate owners implement the motion stylesheet and browser-check helper.
