...
| Blueprint | `render.yaml` proposes `relay-staging`, branch `mvp`, Singapore, Free staging preview, manual deployment, worker disabled by default | Repository configuration, not proof that those resources exist or remain configured that way |
...
## Remote deployment update reconciliation - 2026-09-13

The merged remote commit 17abb8b selected the Free staging blueprint. That choice is retained for a preview; sleeping instances do not establish reliable scheduled work or customer uptime. Auto-deploy, live outbound dispatch and the worker remain disabled in the blueprint. No cloud resource or subscription was changed by this merge.

The older instruction to migrate at application startup is superseded by the implemented no-DDL runtime contract. Run the authorized migration job separately with MIGRATION_DATABASE_URL, then run npm run verify:deploy with the least-privilege runtime connection before manual deployment. The free blueprint omits preDeployCommand; startup and health readiness still refuse an incompatible schema. On an eligible paid plan, the optional pre-deploy step is verification only; do not give the web process migration credentials.

The remote Supabase networking guidance is retained through the connection-mode matrix above: select direct access when reachable or the session pooler for the persistent Node runtime on an IPv4-only network. Do not automatically select transaction pooling for this application. URL-encode special characters in connection credentials, retain certificate verification, and verify the actual network/roles/restore target. PostgreSQL tests require a separate explicitly disposable target; the old staging-database test shortcut and historical test counts are not current acceptance evidence.

Merge verification: formatting passed for 552 files, staged whitespace/conflict checks passed, and the safe runtime-database plus production-foundation suites passed 28 tests with one explicit PostgreSQL skip. No deployed PostgreSQL or cloud change was performed.
