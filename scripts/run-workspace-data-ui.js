...
if (args.length !== 2 || args[0] !== "--playwright-module" || !path.isAbsolute(args[1]) || path.basename(args[1]) !== "index.mjs") {
  ...
  console.log("Workspace data UI target: owned loopback port " + port + ", disposable in-memory SQLite, providers disabled.");
  process.exitCode = await runTestProcess({
    args: ["scripts/verify-workspace-data-ui.js", ...args], env,
