...
if (args.length !== 2 || args[0] !== "--playwright-module" || !path.isAbsolute(args[1]) || path.basename(args[1]) !== "index.mjs") {
  ...
  const env = { ...e2eEnvironment({ ...safeTestEnvironment(), E2E_PORT: String(port) }),
    PUBLIC_APP_ORIGIN: "http://127.0.0.1:" + port, LOG_LEVEL: "error", ENABLE_DEVELOPER_TOOLS: "true" };
  console.log("Email verification UI target: owned loopback port " + port + ", disposable in-memory SQLite, providers disabled.");
  process.exitCode = await runTestProcess({
    args: ["scripts/verify-email-verification-ui.js", ...args], env,
