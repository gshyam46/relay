...
if (args.length !== 2 || args[0] !== "--playwright-module" || !path.isAbsolute(args[1]) || path.basename(args[1]) !== "index.mjs") {
  ...
  console.log("Channel setup UI target: owned loopback port " + port + ", disposable in-memory SQLite, providers disabled.");
  process.exitCode = await runTestProcess({
    args: ["scripts/verify-channel-setup-ui.js", ...args], env,
