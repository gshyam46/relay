...
if (args.length !== 2 || args[0] !== "--playwright-module" || !path.isAbsolute(args[1]) || path.basename(args[1]) !== "index.mjs") {
  ...
  console.log("Analysis UI target: owned loopback port " + port + ", disposable in-memory SQLite, injected provider only.");
  process.exitCode = await runTestProcess({
    args: ["scripts/verify-analysis-jobs-ui.js", ...args], env,
